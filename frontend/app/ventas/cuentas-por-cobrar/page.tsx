...
function AlertBanner({ items }: { items: PendingInvoice[] }) {
  const urgent = items.filter(i => { const d = getDaysUntilDue(i.dueDate); return d !== null && d <= 5; });
  if (urgent.length === 0) return null;
  return (
    <div style={{ background: 'linear-gradient(135deg, rgba(234,88,12,0.12), rgba(220,38,38,0.08))', border: '1px solid rgba(234,88,12,0.4)', borderRadius: '16px', padding: '16px 20px', display: 'flex', alignItems: 'flex-start', gap: '14px', marginBottom: '20px' }}>
      <span style={{ fontSize: '1.5rem', flexShrink: 0 }}>🔔</span>
      <div>
        <p style={{ margin: 0, fontWeight: 700, color: '#c2410c', fontSize: '0.92rem' }}>
          {urgent.length} factura{urgent.length > 1 ? 's' : ''} con fecha límite próxima
        </p>
        <ul style={{ margin: '6px 0 0', padding: '0 0 0 16px', color: '#9a3412', fontSize: '0.82rem', lineHeight: 1.6 }}>
          {urgent.map(inv => {
            const days = getDaysUntilDue(inv.dueDate)!;
            return (
              <li key={inv.id}>
                <strong>{inv.customer}</strong> — Folio {inv.invoiceNumber} —&nbsp;
...
export default function CuentasPorCobrarPage() {
  ...
  const [confirmTarget, setConfirmTarget] = useState<PendingInvoice | null>(null);
  ...
  const [newInvoice, setNewInvoice] = useState({ customer: '', invoiceNumber: '', date: '', paymentMethod: 'PUE', subtotal: 0, iva: 0, amount: 0, dueDate: '' });

  const load = useCallback(async () => {
    try {
      const data = await fetchFromApi('/api/receivables');
      setItems(data.items || []);
  ...
  async function handleConfirm() {
    ...
    } catch { alert('Error al confirmar el cobro'); } finally { setConfirming(false); }
  }
    ...
    try {
      const created = await fetchFromApi('/api/invoices', {
        ...
        body: JSON.stringify({
          ...
          amount: newInvoice.amount, subtotal: newInvoice.subtotal, iva: newInvoice.iva,
          paymentMethod: newInvoice.paymentMethod === 'PPD' ? 'PPD - Pago en parcialidades o diferido' : 'PUE - Pago en una sola exhibición',
          category: 'Ventas', source: 'Cuenta por Cobrar', status: 'pendiente',
          affectBank: false, dueDate: newInvoice.dueDate || null
        })
      });
      setItems(prev => [{ ...created, customer: created.customerName || created.customer }, ...prev]);
      setAddModalOpen(false);
      setAddMode('file');
      setNewInvoice({ customer: '', invoiceNumber: '', date: '', paymentMethod: 'PUE', subtotal: 0, iva: 0, amount: 0, dueDate: '' });
  ...
  async function handleDelete(item: PendingInvoice) {
    if (!confirm(`¿Eliminar la factura de ${item.customer}?`)) return;
    try {
  ...
  if (loading) return (
    <WorkspaceShell active="/ventas/cuentas-por-cobrar" eyebrow="Ventas" title="Cuentas por Cobrar" subtitle="Cargando...">
  ...
  return (
    <WorkspaceShell active="/ventas/cuentas-por-cobrar" eyebrow="Ventas" title="Cuentas por Cobrar" subtitle="Facturas pendientes de cobro. Confirma el pago para registrar el ingreso en banco.">
      <section className="stack">
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px', marginBottom: '8px' }}>
          {[
            { label: 'Facturas pendientes', value: filtered.length.toString(), icon: '🧾', accent: '#0d9488' },
            { label: 'Total por cobrar', value: money(totalPendiente), icon: '💰', accent: '#7c3aed' },
        ...
        <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
          <input className="form__input" placeholder="🔍 Buscar por cliente o folio..." value={search} onChange={e => setSearch(e.target.value)} style={{ flex: 1, maxWidth: '380px' }} />
          <button className="button button--primary" onClick={() => setAddModalOpen(true)} style={{ marginLeft: 'auto', whiteSpace: 'nowrap' }}>+ Agregar Cuenta por Cobrar</button>
        </div>
        ...
        {filtered.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '60px 20px', background: 'var(--surface)', borderRadius: '16px', border: '1px dashed var(--line)' }}>
            ...
            <p style={{ color: '#64748b', fontSize: '1rem', fontWeight: 600 }}>{search ? 'Sin resultados.' : 'No hay facturas pendientes de cobro.'}</p>
          </div>
        ) : (
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.875rem' }}>
                <thead>
                  <tr style={{ background: 'var(--surface-alt, #f8fafc)', borderBottom: '1px solid var(--line)' }}>
                    {['Cliente', 'Folio', 'Fecha', 'Método', 'Vence', 'Subtotal', 'IVA', 'Total', 'Acciones'].map(h => (
                ...
                <tbody>
                  {filtered.map((item, idx) => {
                    ...
                    return (
                      <tr key={item.id} style={{ borderBottom: '1px solid var(--line)', background: isOverdue ? 'rgba(220,38,38,0.03)' : isUrgent ? 'rgba(234,88,12,0.03)' : idx % 2 === 0 ? 'transparent' : 'rgba(0,0,0,0.01)' }}>
                        ...
                        <td style={{ padding: '14px 16px', fontWeight: 800, color: '#0d9488', whiteSpace: 'nowrap' }}>{money(item.amount)}</td>
                        <td style={{ padding: '14px 16px' }}>
                          <div style={{ display: 'flex', gap: '8px' }}>
                            <button onClick={() => { setConfirmTarget(item); setBankAccount('banorte'); }} style={{ padding: '7px 14px', borderRadius: '10px', border: 'none', cursor: 'pointer', background: 'linear-gradient(135deg, #0d9488, #0891b2)', color: '#fff', fontWeight: 700, fontSize: '0.8rem', whiteSpace: 'nowrap', boxShadow: '0 2px 8px rgba(13,148,136,0.3)' }}>✅ Cobrado</button>
      ...
      <Modal isOpen={!!confirmTarget} onClose={() => setConfirmTarget(null)} title="Confirmar cobro" description="Selecciona la cuenta donde entró el pago." size="md">
        {confirmTarget && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            <div style={{ background: 'rgba(13,148,136,0.06)', border: '1px solid rgba(13,148,136,0.2)', borderRadius: '14px', padding: '18px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                {[['Cliente', confirmTarget.customer], ['Folio', confirmTarget.invoiceNumber], ['Método', confirmTarget.paymentMethod?.includes('PPD') ? 'PPD' : 'PUE'], ['Total', money(confirmTarget.amount)]].map(([k, v]) => (
            ...
            <div>
              <p style={{ margin: '0 0 10px', fontSize: '0.85rem', fontWeight: 700 }}>💳 ¿A qué cuenta entra el dinero?</p>
              <div style={{ display: 'flex', gap: '12px' }}>
                {([['banorte', '🏦', banorteAlias, '#0d9488'], ['bbva', '🏛️', bbvaAlias, '#1d4ed8']] as const).map(([val, icon, name, color]) => (
                  <label key={val} style={{ flex: 1, display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer', padding: '12px 16px', borderRadius: '12px', border: `2px solid ${bankAccount === val ? color : '#e2e8f0'}`, background: bankAccount === val ? `${color}0d` : '#fff', transition: 'all 0.2s' }}>
                    <input type="radio" name="confirm-bank" value={val} checked={bankAccount === val} onChange={() => setBankAccount(val)} style={{ accentColor: color }} />
            ...
            <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end' }}>
              ...
              <button className="button button--primary" onClick={handleConfirm} disabled={confirming} style={{ background: 'linear-gradient(135deg, #0d9488, #0891b2)', minWidth: '140px' }}>
                {confirming ? 'Procesando...' : '✅ Confirmar Cobro'}
              </button>
            </div>
          </div>
        )}
      ...
      {/* Modal agregar */}
      <Modal isOpen={addModalOpen} onClose={() => { setAddModalOpen(false); setAddMode('file'); }} title="Nueva Cuenta por Cobrar" description="No afecta el banco hasta confirmar el pago." size="lg">
        {/* Tabs */}
        ...
        {addMode === 'file' ? (
          <InvoiceUploader
            title="Escanear factura por cobrar"
            description="Sube XML o PDF para autollenar cliente, folio y montos."
            actionLabel="Por Cobrar"
            accent="rgba(13,148,136,0.25)"
            showCategorySelector={false}
            isSale={true}
            onParsed={handleParsedFile}
          />
        ) : (
        <form className="stack" onSubmit={handleAddInvoice} style={{ marginTop: '10px' }}>
          <label className="form__row"><span className="form__label">Cliente</span><input required className="form__input" placeholder="Nombre del cliente" value={newInvoice.customer} onChange={e => setNewInvoice({ ...newInvoice, customer: e.target.value })} /></label>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
            <label className="form__row"><span className="form__label">Folio</span><input className="form__input" placeholder="A-1234" value={newInvoice.invoiceNumber} onChange={e => setNewInvoice({ ...newInvoice, invoiceNumber: e.target.value })} /></label>
            <label className="form__row"><span className="form__label">Fecha</span><input required className="form__input" type="datetime-local" value={newInvoice.date} onChange={e => setNewInvoice({ ...newInvoice, date: e.target.value })} /></label>
          </div>
          <label className="form__row">
            <span className="form__label">Método de Pago</span>
            <select className="form__select" value={newInvoice.paymentMethod} onChange={e => setNewInvoice({ ...newInvoice, paymentMethod: e.target.value, dueDate: e.target.value === 'PUE' ? '' : newInvoice.dueDate })}>
          ...
          {newInvoice.paymentMethod === 'PPD' && (
            <div style={{ background: 'rgba(124,58,237,0.06)', border: '1px solid rgba(124,58,237,0.2)', borderRadius: '12px', padding: '14px 16px' }}>
              ...
              <input className="form__input" type="date" value={newInvoice.dueDate} onChange={e => setNewInvoice({ ...newInvoice, dueDate: e.target.value })} style={{ borderColor: 'rgba(124,58,237,0.3)' }} />
            </div>
          )}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '15px' }}>
            <label className="form__row"><span className="form__label">Subtotal</span><input required type="number" step="0.01" className="form__input" value={newInvoice.subtotal || ''} onChange={e => { const sub = parseFloat(e.target.value) || 0; const iva = parseFloat((sub * 0.16).toFixed(2)); setNewInvoice({ ...newInvoice, subtotal: sub, iva, amount: parseFloat((sub + iva).toFixed(2)) }); }} /></label>
            <label className="form__row"><span className="form__label">IVA (16%)</span><input type="number" step="0.01" className="form__input" value={newInvoice.iva || ''} onChange={e => setNewInvoice({ ...newInvoice, iva: parseFloat(e.target.value) || 0, amount: parseFloat((newInvoice.subtotal + (parseFloat(e.target.value) || 0)).toFixed(2)) })} /></label>
            <label className="form__row"><span className="form__label">Total</span><input required type="number" step="0.01" className="form__input" value={newInvoice.amount || ''} onChange={e => setNewInvoice({ ...newInvoice, amount: parseFloat(e.target.value) || 0 })} /></label>
          </div>
          <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end', marginTop: '8px' }}>
            ...
            <button type="submit" className="button button--primary">Registrar Cuenta por Cobrar</button>
          </div>
        </form>
        )}
      </Modal>
